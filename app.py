...
# nest_asyncio HANYA untuk Colab/Jupyter. Di server (Railway) bikin bentrok dgn uvloop.
import sys as _sys
if "google.colab" in _sys.modules or "ipykernel" in _sys.modules:
    nest_asyncio.apply()
...
if __name__=="__main__":
    IS_COLAB = ("google.colab" in sys.modules) or ("ipykernel" in sys.modules)
    if IS_COLAB:
        ...
        # Railway/VPS — FastAPI. Paksa loop 'asyncio' (bukan uvloop) biar tidak bentrok.
        log.info(f"Mode: SERVER FastAPI port {C.PORT}")
        config = uvicorn.Config(app, host="0.0.0.0", port=C.PORT, log_level="warning", loop="asyncio")
